            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              {pendingReqs.map((req) => (
                <div key={req.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: "0.5px solid var(--line)" }}>
                  <div style={{ width: 36, height: 36, borderRadius: 99, background: COLORS[req.senderId?.length % COLORS.length] ?? "#888", display: "inline-flex", alignItems: "center", justifyContent: "center", color: "white", fontSize: "var(--text-base)", fontWeight: 800, flexShrink: 0 }}>{req.senderName?.[0]?.toUpperCase() ?? "?"}</div>
